---
title: "Social Network - sin resolver"
...
```{r}
g <- graph_from_data_frame(___________, directed=TRUE, vertices=___________)
class(___________) #chequeamos que esta bien el tipo de grafico
```

3.a. Conozcamos un poco más de info sobre nuestro grafico con print()
```{r}
print(___________, e=TRUE, v=TRUE)
```

3.b. Plotiemos el gráfico con la función *plot()*!
```{r}
___________(g)
```

3.d. Veamos info sobre los nodos con *V()*:
```{r}
___________(g)
```

3.e. Veamos info sobre las relaciones entre usuarios con *E()*:
```{r}
___________(g)
```
...
```{r}
plot(g, vertex.label = NA, vertex.color = "___________")
```

4.b.cambiamos el color de los nodos según la columna "colors":
```{r}
nodos <- nodos %>% left_join(data_usuarios, by = "name")

g <- ___________(links, directed=TRUE, vertices=nodos)

plot(___________, vertex.label = NA)
```

4.c. Cambiar el color de los usuarios y de las flechas con "vertex.label.color" y "edge.color":
```{r}
plot(g, vertex.label.color = "___________", edge.color = "green")
```

#5. Degree

5.a. Calculemos el "out-degree" con la funcion "degree()"
```{r}
g.outd <- ___________(g, mode = c("___________"))
...
```{r}
g.ind <- degree(g, mode = c("___________"))
my.label<- names(g.ind)
my.label[which(log(g.ind+1) < ___________)]<- "" 
my.label2<- my.label

plot(g,  vertex.label = ___________, vertex.label.cex = 1.3)
```
...
users_wc <- membership(___________)
...
#EXPORTAR
13. Exportar con *jpeg()*, *plot.igraph()* y *dev.off()*:

```{r}
___________(file = "Network.jpg", width = 4000, height = 4000, units = "px", pointsize = 75, quality = 95, 
     bg = "white", res = NA, family = "", restoreConsole = TRUE, type = c("windows", "cairo"))

___________(g, vertex.label=my.label2, layout=l, vertex.size=log(g.ind+1)/3.5, vertex.label.color="Black",
...
nodes <- nodos %>% rename(id = name, label = screen_name)
 ...
 visNetwork(___________, ___________, main = paste0("Interaccion en Twitter")) %>%
      visIgraphLayout() %>%
      ___________(
        shape = "dot",
     ...
     ___________(
      ...
      visOptions(highlightNearest = list(enabled = T, degree = 1,
                                         hover = T)) %>%
      ___________(enabled = F, solver = "repulsion", repulsion = list(nodeDistance = 1000)) %>%
